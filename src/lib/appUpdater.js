...
// Collect PIDs of all bee-router-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from bee-router install dir or wrapper cli.js
        const isAppProcess = lower.includes("bee-router") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("bee-router") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "bee-router");
  }
  return path.join(os.homedir(), ".bee-router");
}
...
// Resolve npx/bee-router binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx @tonamson2/bee-router` — works regardless of global bin path changes after npm i -g
