...
export const CLI_TOOLS = {
  ...
  amp: {
    ...
    notes: [
      { type: "info", text: "Use BeeRouter model aliases to keep Amp shorthand mappings stable across provider updates." },
    ...
    guideSteps: [
      ...
      { step: 5, title: "Add Shorthands", desc: "Map Amp shorthand names such as g25p or cs45 to BeeRouter aliases in your local config." },
    ],
    codeBlock: {
      language: "bash",
...
# cs45 -> cc/claude-sonnet-4-5-20250929`,
  ...
  qwen: {
    ...
    color: "#10B981",
    description: "Alibaba Qwen Code CLI — supports OpenAI, Anthropic & Gemini providers via BeeRouter",
    docsUrl: "https://qwenlm.github.io/qwen-code-docs/en/users/configuration/model-providers/",
    configType: "guide",
    defaultCommand: "qwen",
    notes: [
      { type: "info", text: "Qwen Code supports multiple provider types (openai, anthropic, gemini) via modelProviders in settings.json. BeeRouter works as an OpenAI-compatible endpoint." },
      { type: "info", text: "Any model available in BeeRouter can be used — not just Qwen models. Select from Qwen, Claude, Gemini, GPT, and more." },
      { type: "warning", text: "Config path: Linux/macOS ~/.qwen/settings.json • Windows %USERPROFILE%\\.qwen\\settings.json" },
      { type: "error", text: "Qwen OAuth free tier was discontinued on 2026-04-15. Use BeeRouter with alicode/openrouter/anthropic/gemini providers instead." },
    ],
  ...
  "deepseek-tui": {
    ...
    notes: [
      { type: "info", text: "DeepSeek TUI uses ~/.deepseek/config.toml for configuration. BeeRouter will update the provider to 'openai' mode with your base_url, api_key, and model." },
  ...
  jcode: {
    ...
    notes: [
      ...
      {
        type: "info",
        text: "Configure bee-router as an OpenAI-compatible provider to route all jcode requests through bee-router's optimization layer."
      },
      {
        type: "warning",
  ...
  agy: {
    ...
    notes: [
      {
        type: "info",
        text: "agy Gemini-API mode only accepts catalog names like Gemini 3.1 Pro. BeeRouter writes that to settings.json and remaps requests to the model you Apply.",
      },
      {
        type: "info",
  ...
  "grok-build": {
    ...
    notes: [
      {
        type: "info",
        text: "Grok Build uses ~/.grok/config.toml. BeeRouter writes a [model.bee-router] custom model and sets it as the default.",
      },
      {
        type: "info",
        text: "After Apply, run grok (or /model bee-router) to use the routed model. Switch back anytime with /model grok-build.",
      },
      {
        type: "warning",
...
devin --version`,
  ...
  opendesign: {
    ...
    configType: "guide",
    notes: [
      { type: "info", text: "OpenDesign ships as a plugin/skills pack installed into Claude Code, Cursor, OpenAI Codex, Gemini CLI, or OpenCode. It inherits the host agent's model config, so once your host points at BeeRouter, /opendesign design sessions route through BeeRouter automatically — no extra env vars needed." },
    ...
    guideSteps: [
      ...
      { step: 2, title: "No config needed", desc: "OpenDesign runs inside your host agent and uses its model config. If the host already routes through BeeRouter, /opendesign traffic does too." },
