...
Integrate BeeRouter with Cline VSCode extension to route your AI requests through BeeRouter's intelligent routing system.

## Prerequisites

- Visual Studio Code installed
- Cline extension installed from VSCode marketplace
- BeeRouter running locally or cloud endpoint configured
...
Set the base URL to your BeeRouter endpoint:
...
**Steps:**
1. In the **Base URL** field, enter your BeeRouter endpoint
...
3. The key should start with `sk-bee-router-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your BeeRouter configuration
...
4. Cline will use BeeRouter to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using BeeRouter
...
3. Get AI-powered explanations through BeeRouter

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use BeeRouter to understand context and make changes
...
1. Verify BeeRouter is running: `curl http://localhost:20128/health`
...
2. Verify your BeeRouter instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check BeeRouter logs for any errors

## Advanced Configuration

### Using Cloud Endpoint

To use BeeRouter cloud endpoint instead of localhost:

1. In Cline settings, set Base URL to: `https://9router.com`
2. Make sure you have configured your API key in the BeeRouter cloud dashboard
...
## Integration with BeeRouter Features

### Model Routing

BeeRouter automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, BeeRouter automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through BeeRouter dashboard:
- Total requests
- Token usage
- Cost per model
