...
async function buildCodexHeader() {
  ...
  const { installed, hasBeeRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasBeeRouter, settings } = result.data;
  ...
  // Extract BeeRouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:BeeRouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract BeeRouter provider config
  const provider = settings?.models?.providers?.["bee-router"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("bee-router/") ? primary.replace("bee-router/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasBeeRouter, opencode } = result.data;
...
async function buildAgyHeader() {
  ...
  const { installed, hasBeeRouter, settings, env } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use BeeRouter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
