# High-Tech Terminal Deck Login Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign `/login` into a centered Developer Terminal Deck UI with macOS window controls, CLI prompt stream, password visibility toggle, SSO support, and copyable commands.

**Architecture:** Centered single-card terminal window with ambient honeycomb background, top navigation bar, CLI-style authentication form, and full backward compatibility with all auth APIs.

**Tech Stack:** Next.js 16 (App Router), React 19, Tailwind CSS v4, Material Symbols icons, next/image.

## Global Constraints

- Preserve all existing auth states and API endpoints (`/api/auth/status`, `/api/auth/login`, `/api/auth/oidc/start`, `/api/auth/saml/start`, `/api/settings`).
- Use official logo `/logo.png?v=2` with `next/image`.
- Ensure 100% English copy throughout the interface.
- Responsive centering across all screen sizes.

---

### Task 1: Implement High-Tech Terminal Deck in `src/app/login/page.js`
...
- [ ] **Step 1: Replace `src/app/login/page.js` with High-Tech Terminal Deck UI**
...
git commit -m "feat(login): implement high-tech terminal deck login UI"
```
