# Landing Page Login Button Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a "Sign in" button to the Landing Page header navigation for both desktop and mobile views directing to `/login`.

**Architecture:** Update `Navigation.js` in `src/app/landing/components/` by adding the "Sign in" button next to "Open Dashboard" in desktop header, and inside the mobile drawer menu.

**Tech Stack:** Next.js (App Router), React, Tailwind CSS.

## Global Constraints

- Keep existing design tokens (Amber `#FFC700`, dark background `#0D0E12`, border `#282B37`).
- Keep existing routing paths (`/login`, `/dashboard`).
- Preserve responsiveness across mobile and desktop breakpoints.

---

### Task 1: Add "Sign in" button to Desktop and Mobile Navigation
...
- Produces: Visual "Sign in" buttons that trigger `router.push("/login")`

- [ ] **Step 1: Update `src/app/landing/components/Navigation.js`**

Update `Navigation.js` to render the "Sign in" button on desktop and mobile menus:
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-[#282B37] bg-[#0D0E12]/95 backdrop-blur-md">
          <div className="flex flex-col gap-4 p-6">
            ...
            <div className="flex flex-col gap-2 pt-2 border-t border-[#282B37]">
              <button 
                onClick={() => {
...
Run lint or test build to verify no syntax errors.
...
```bash
git add src/app/landing/components/Navigation.js docs/superpowers/specs/2026-08-18-landing-login-button-design.md docs/superpowers/plans/2026-08-18-landing-login-button.md
git commit -m "feat(landing): add sign in button to header navigation"
```
