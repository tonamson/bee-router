...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-bee-router.env");
};
...
const hasBeeRouterConfig = (config) => {
  ...
  if (providers["bee-router"]) return true;
...
export async function GET() {
  ...
  const hasBeeRouter = hasBeeRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_BEE_ROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile bee-router",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["bee-router"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_BEE_ROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "bee-router configuration removed from jcode",
    });
  } catch (error) {
