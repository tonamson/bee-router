<div align="center">
  ...
  <br/>
  <img src="../images/bee-router.png?1" alt="Bảng điều khiển bee-router" width="800"/>
...
**BeeRouter giải quyết vấn đề này:**
...
Gói kho lưu trữ này là riêng tư (`bee-router-app`), vì vậy việc chạy từ nguồn/Docker là cách phát triển cục bộ mặc định.
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - BeeRouter + Claude Code MIỄN PHÍ
  
[![Thiết lập BeeRouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập BeeRouter
...
BeeRouter hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → BeeRouter dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân BeeRouter **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của BeeRouter

**Thực tế Thanh toán BeeRouter:**

✅ **Phần mềm BeeRouter = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **BeeRouter không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, BeeRouter chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị BeeRouter tính phí không?</b></summary>

**Không.** BeeRouter là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, BeeRouter chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân BeeRouter** → **Không bao giờ tính phí bất cứ thứ gì, ever**

BeeRouter là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
BeeRouter chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và BeeRouter giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải BeeRouter.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - BeeRouter giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của BeeRouter ngăn chặn các khoản phí bất ngờ:
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và BeeRouter sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. BeeRouter theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển bee-router]
...
> **Lưu ý:** OpenClaw chỉ hoạt động với BeeRouter cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
# Hoặc sử dụng PM2
npm install -g pm2
pm2 start --name bee-router -- start
pm2 save
pm2 startup
```
...
docker run -d \
  --name bee-router \
  -p 20128:20128 \
  --env-file /root/dev/bee-router/.env \
  -v bee-router-data:/app/data \
  -v bee-router-usage:/root/.bee-router \
  bee-router
```
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | Tự động sinh (`~/.bee-router/jwt-secret`) | Bí mật ký JWT cho cookie xác thực bảng điều khiển (đặt để chia sẻ giữa nhiều instance) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.bee-router` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
...
- Lịch sử sử dụng và log: `~/.bee-router/usage.json` và `~/.bee-router/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.bee-router` và độc lập với `DATA_DIR`.
...
- Runtime BeeRouter cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- **GitHub**: [github.com/decolua/9](https://github.com/tonamson/bee-router)
...
Cảm ơn tất cả những người đã đóng góp giúp BeeRouter tốt hơn!

[![Contributors](https://contrib.rocks/image?repo=tonamson/bee-router&max=100&columns=20&anon=1)](https://github.com/tonamson/bee-router/graphs/contributors)

---

## 📊 Star Chart

[![ Chart](https://starchart.cc/tonamson/bee-router.svg?variant=adaptive)](https://starchart.cc/tonamson/bee-router)
...
Xem [Pull Requests](https://github.com/tonamson/bee-router/pulls) để biết hướng dẫn chi tiết.

---

## 🔀 Forks

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Một fork TypeScript đầy đủ tính năng của BeeRouter. Thêm 36+ nhà cung cấp, tự động dự phòng 4 tầng, API đa phương thức (hình ảnh, embedding, âm thanh, TTS), circuit breaker, bộ nhớ đệm ngữ nghĩa, đánh giá LLM và bảng điều khiển được tinh chỉnh. 368+ bài kiểm tra đơn vị. Có sẵn qua npm và.
