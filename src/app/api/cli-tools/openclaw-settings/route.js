...
// Check if settings has BeeRouter config
const hasBeeRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["bee-router"];
};

// Read per-agent models.json and return current model id (without "bee-router/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["bee-router"]?.models;
...
// POST - Update BeeRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `bee-router/${model}`;

    // Remove all old bee-router/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("bee-router/"))
    ...
    // Add fresh bee-router models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`bee-router/${m}`] = {};
    });

    // Remove old bee-router model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("bee-router/")) {
    ...
    // Update models.providers.bee-router with all models
    settings.models.providers["bee-router"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `bee-router/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove BeeRouter settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove BeeRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["bee-router"];
    ...
    // Remove bee-router models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("bee-router/"));
    ...
    // Reset agents.defaults.model.primary if it uses bee-router
    if (settings.agents?.defaults?.model?.primary?.startsWith("bee-router/")) {
