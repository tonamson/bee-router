# BeeRouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

BeeRouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[BeeRouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.bee-router/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.bee-router/usage.json`, `~/.bee-router/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.bee-router/db.json`)
- usage stats: `~/.bee-router/usage.json`
- request log lines: `~/.bee-router/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[BeeRouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.bee-router` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/bee-router && npm run build`
- Build Docker image: `cd /root/dev/bee-router && docker build -t bee-router .`
