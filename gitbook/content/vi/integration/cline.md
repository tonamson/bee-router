...
Tích hợp BeeRouter với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của BeeRouter.
...
Đặt base URL tới endpoint BeeRouter:

**Cho BeeRouter cục bộ:**
```
...
**Các bước:**
1. Trong field **Base URL**, nhập endpoint BeeRouter
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key BeeRouter của bạn
2. Bạn có thể tìm API key trong BeeRouter dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-bee-router-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình BeeRouter
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng BeeRouter để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua BeeRouter
...
3. Nhận giải thích AI qua BeeRouter

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng BeeRouter để hiểu context và thực hiện thay đổi
...
2. Xác minh BeeRouter instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs BeeRouter để xem lỗi
...
2. Đảm bảo bạn đã cấu hình API key trong BeeRouter cloud dashboard
...
## Tích hợp với Tính năng BeeRouter

### Định tuyến Model

BeeRouter tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, BeeRouter tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua BeeRouter dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
