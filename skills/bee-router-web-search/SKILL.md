---
name: bee-router-web-search
description: Web and X search via BeeRouter /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity / Xquik. Use when the user wants to search the web, find articles, or search public X posts.
---

# BeeRouter — Web Search
...
```bash
curl $BEE_ROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$BEE_ROUTER_URL/v1/models/info?id=tavily/search"
```
...
```bash
curl -X POST $BEE_ROUTER_URL/v1/search \
  -H "Authorization: Bearer $BEE_ROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"tavily","query":"BeeRouter open source","max_results":5}'
```

JS:

```js
const r = await fetch(`${process.env.BEE_ROUTER_URL}/v1/search`, {
  method: "POST",
...
Add the Xquik API key in BeeRouter's provider settings. Xquik charges 1 credit per returned post. Continue a search by passing `pagination.next_cursor` as `provider_options.cursor`.
