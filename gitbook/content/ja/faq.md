...
**BeeRouterは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、BeeRouter自体は100%無料でオープンソースです。**
...
**BeeRouterはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## BeeRouterをセルフホストできますか?

**はい! BeeRouterは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.bee-router`)
...
**はい、BeeRouterはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.bee-router`(またはカスタム`DATA_DIR`)にローカル保存
- BeeRouterサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/tonamson/bee-router/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.bee-router`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/tonamson/bee-router/blob/main/CONTRIBUTING.md)を参照。
