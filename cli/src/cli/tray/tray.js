...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `BeeRouter (Port ${port})`, tooltip: "Server is running", enabled: false },
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[bee-router] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[bee-router] tray init error: ${err.message}\n`);
    return null;
  }
}
