...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.bee-router/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/bee-router/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[bee-router][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[bee-router][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[bee-router][runtime] chmod tray bin failed: ${e.message}`);
  }
}
