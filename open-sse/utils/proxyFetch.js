...
export async function proxyAwareFetch(url, options = {}, proxyOptions = null) {
  ...
  const isStream = options.headers?.Accept === "text/event-stream" || options.headers?.accept === "text/event-stream";
  if (isAnthropic && !isStream) {
    const gs = await getGotScraping();
    if (gs) {
      try {
        const res = await gs(url, options);
        if (res) {
          ...
          return new Response(res.rawBody, {
            status: res.statusCode,
            statusText: res.statusMessage || "",
            headers: resHeaders,
          });
        ...
        console.warn(`[ProxyFetch] got-scraping request failed, fallback to native fetch: ${err.message}`);
      }
    }
  }
