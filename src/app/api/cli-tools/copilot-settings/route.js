...
const hasBeeRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "BeeRouter");
};

const getBeeRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "BeeRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getBeeRouterEntry(config);
...
// POST - Apply BeeRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing BeeRouter entry or append
    const idx = config.findIndex((e) => e.name === "BeeRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove BeeRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "BeeRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "BeeRouter removed from Copilot config",
    });
  } catch (error) {
