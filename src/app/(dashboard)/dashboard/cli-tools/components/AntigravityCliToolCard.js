...
const ENDPOINT = "/api/cli-tools/agy-settings";

export default function AntigravityCliToolCard({
  tool,
  isExpanded,
  onToggle,
  hasActiveProviders,
  ...
  const [agyStatus, setAgyStatus] = useState(initialStatus || null);
  ...
  const getConfigStatus = () => {
    if (!agyStatus?.installed) return null;
    const url = agyStatus.env?.GOOGLE_GEMINI_BASE_URL;
    if (!agyStatus.settings?.modelProvider || !url) return "not_configured";
    if (matchKnownEndpoint(url, { tunnelPublicUrl, tailscaleUrl })) return "configured";
    return "other";
  };
  ...
  useEffect(() => {
    if (initialStatus) setAgyStatus(initialStatus);
  }, [initialStatus]);

  useEffect(() => {
    if (isExpanded) {
      if (!agyStatus) checkStatus();
      fetchModelAliases();
    }
  }, [isExpanded]);
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      setAgyStatus({ installed: false, error: error.message });
    } finally {
      setChecking(false);
    }
  };
  ...
  const getEffectiveBaseUrl = () => (customBaseUrl || getLocalBaseUrl()).replace(/\/+$/, "");
    ...
    <Card padding="xs" className="overflow-hidden">
      <div className="flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
        <div className="flex min-w-0 items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <Image src={tool.image || "/providers/antigravity.png"} alt={tool.name} width={32} height={32} className="size-8 object-contain rounded-lg" sizes="32px" onError={(e) => { e.target.style.display = "none"; }} loading="lazy" decoding="async" />
          </div>
      ...
      {isExpanded && (
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          {!checking && agyStatus && !agyStatus.installed && (
          ...
          {!checking && agyStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {agyStatus?.env?.GOOGLE_GEMINI_BASE_URL && (
                  <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                    <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">Current</span>
                    <span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                    <span className="min-w-0 truncate rounded bg-surface/40 px-2 py-2 text-xs text-text-muted sm:py-1.5">
                      {agyStatus.env.GOOGLE_GEMINI_BASE_URL} · {agyStatus.settings?.model || "no model"}
                    </span>
                  </div>
                )}
