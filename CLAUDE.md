...
- DB file location resolves via `src/lib/db/paths.js` (`DATA_DIR`, else `~/.bee-router/`).
- Usage/logs (`src/lib/usageDb.js`, `usage.json` + `log.txt`) still live under `~/.bee-router` and do **not** follow `DATA_DIR`.
