---
name: bee-router-web-fetch
description: Fetch URL → markdown / text / HTML via BeeRouter /v1/web/fetch using Ollama Cloud / Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---
...
```bash
curl $BEE_ROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$BEE_ROUTER_URL/v1/models/info?id=firecrawl/fetch"
```
...
`POST $BEE_ROUTER_URL/v1/web/fetch`
...
```js
const r = await fetch(`${process.env.BEE_ROUTER_URL}/v1/web/fetch`, {
  method: "POST",
