...
Guía detallada de instalación de BeeRouter con consejos de solución de problemas.
...
- ✅ Comando simple: `bee-router`
- ✅ Auto-actualizaciones con `npm update -g @tonamson2/bee-router`
...
3. Se crea el directorio de datos en `~/.bee-router`
...
**Ubicación por defecto:** `~/.bee-router`
...
**Error:**
```
Error: EACCES: permission denied, mkdir '/usr/local/lib/node_modules/bee-router'
```
...
**Problema:** BeeRouter usa demasiada RAM
...
```bash
npm install -g pm2
pm2 start bee-router --name bee-router
pm2 save
```
...
```bash
docker pull bee-router/bee-router:latest

docker run -d \
  -p 20128:20128 \
  -e JWT_SECRET="your-secure-secret" \
  -e INITIAL_PASSWORD="your-password" \
  -v bee-router-data:/root/.bee-router \
  --name bee-router \
  bee-router/bee-router:latest
```
...
nano ~/.bashrc  # o ~/.zshrc
# Eliminar exports relacionados con bee-router
```
